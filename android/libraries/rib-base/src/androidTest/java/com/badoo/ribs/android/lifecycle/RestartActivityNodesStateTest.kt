...
class RestartActivityNodesStateTest(private val test: Pair<When, Then>) : BaseNodesTest() {
    ...
    companion object {
        ...
        fun data() = listOf(
            When(pushConfiguration1 = AttachNode1, pushConfiguration2 = AttachNode2)
                to Then(node1 = NodeState(attached = false, viewAttached = false), node2 = NodeState(attached = true, viewAttached = true)),

            When(pushConfiguration1 = AttachNode1, pushConfiguration2 = AttachNode2AsOverlay)
                to Then(node1 = NodeState(attached = false, viewAttached = true), node2 = NodeState(attached = true, viewAttached = true)),

            When(pushConfiguration1 = AttachNode1, pushConfiguration2 = AttachNode2AsDialog)
                to Then(node1 = NodeState(attached = false, viewAttached = false), node2 = NodeState(attached = true, viewAttached = false)),

            When(pushConfiguration1 = AttachNode1, pushConfiguration2 = AttachNode2AsDialogAndOverlay)
                to Then(node1 = NodeState(attached = false, viewAttached = true), node2 = NodeState(attached = true, viewAttached = false)),
