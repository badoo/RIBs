...
class RequestCodeBasedEventStreamImplTest {
    ...
    fun `publish - no clients listening for the response - calls unhandled request code handler`() {
        ...
        verify(RIBs.errorHandler).handleNoRequestCodeListenersError(
            eq(1),
            eq(internalRequestCode),
            any(),
            eq(event)
        )
    }
    ...
    fun `publish - there are clients listening for the response - does not call unhandled request code handler`() {
        ...
        verify(RIBs.errorHandler, never()).handleNoRequestCodeListenersError(
            any(),
            any(),
            any(),
            any()
    ...
    class TestEvent(override val requestCode: Int) :
        RequestCodeBasedEventStream.RequestCodeBasedEvent

    class TestRequestCodeBasedEventStream :
        RequestCodeBasedEventStreamImpl<TestEvent>(requestCodeRegistry = RequestCodeRegistry(null)) {
