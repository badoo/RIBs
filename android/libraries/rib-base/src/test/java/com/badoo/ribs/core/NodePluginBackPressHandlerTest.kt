...
class NodePluginBackPressHandlerTest : NodePluginTest() {

    @Test
    fun `BackPressHandler plugin receives handleBackPressBeforeDownstream()`() {
        val (node, plugins) = testPlugins<BackPressHandler>(1)

        node.handleBackPress()
        verify(plugins.first()).handleBackPressBeforeDownstream()
    }
