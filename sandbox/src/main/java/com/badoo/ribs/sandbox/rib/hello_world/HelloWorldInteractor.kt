...
class HelloWorldInteractor(
    ...
    private val dummyViewInput = BehaviorRelay.createDefault(
        ViewModel(
            "My id: " + buildParams.identifier.uuid.toString()
    ...
    override fun onViewCreated(view: HelloWorldView, viewLifecycle: Lifecycle) {
        viewLifecycle.startStop {
            ...
            bind(activityStarter.events(this@HelloWorldInteractor).rx3() to activityResultConsumer)
        ...
        if (it.requestCode == REQUEST_CODE_OTHER_ACTIVITY) {
            if (it.resultCode == Activity.RESULT_OK) {
                dummyViewInput.accept(
                    ViewModel(
                        "Data returned: " + it.data?.getIntExtra(
                            OtherActivity.KEY_RETURNED_DATA,
                            -1
                        )?.toString()
                    )
                )
